...
const syntaxrules: SyntaxRule[] = [
  {
    type: "NUM",
    rule: ["LIT_HEX"],
    parse: (x) => parseInt(x[0].value.replace(/_/, ""), 16),
  },
  {
    type: "NUM",
    rule: ["LIT_DEC"],
    parse: (x) => parseInt(x[0].value.replace(/_/, ""), 10),
  },
  {
    type: "NUM",
    rule: ["LIT_BIN"],
    parse: (x) => parseInt(x[0].value.replace(/_/, ""), 2),
  },

  { type: "LABEL", rule: ["IDENT", ":"], parse: (x) => x[0].value },
  {
    type: "SHREG",
    rule: ["IDENT", "SHIFT", "NUM"],
    parse: (x) => [x[0].value, x[1].value, x[2].value],
  },
...
function parseline(tokens: Token[]): Instruction | null {
  ...
  switch (instr) {
    ...
    default:
      return isa.parseinstr(instr, tokens);
  }
}
